...
tau = 10;    % Time scale of anti-inflammatory cytokines
chi = 1.75;  % Chemotaxis coefficient
...
m = zeros(Nx, Nt); % Macrophage density
c = zeros(Nx, Nt); % Chemokine concentration
...
%% Creation of the space-time figure for m
figure;
imagesc(xi, tj, m'); % Transposed by m to have time on the y-axis

t_start = 9000; % Display start time
idx_start = find(tj >= t_start, 1); % Find the index corresponding to t = t_start
imagesc(xi, tj(idx_start:end), m(:, idx_start:end)'); % Only show data from t=t_start onwards

xlabel('Spatial position x');
ylabel('Time t');
title('Time evolution of m(x, t)');
colorbar;
colormap(jet);
axis tight;